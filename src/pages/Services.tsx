          ...
          <aside className="lg:w-64 space-y-6">
            ...
            <form onSubmit={handleSearch} className="relative">
              <input
                ...
                placeholder="Поиск..."
                className="w-full px-4 py-3 pl-10 bg-slate-800 border border-slate-600 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-500"
              />
            ...
            {/* Categories */}
            <div className="bg-slate-800/30 backdrop-blur-lg border border-slate-700 rounded-xl p-4">
              <h3 className="text-white font-semibold mb-3">Категории</h3>
              <div className="space-y-2">
                <button
                  onClick={() => { setSelectedCategory(''); setSelectedSubcategory(''); }}
                  className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${
                    !selectedCategory ? 'bg-slate-600/30 text-slate-300' : 'text-slate-300 hover:bg-slate-700/50'
                  }`}
                >
                  Все категории
                </button>
                  ...
                  <div key={category.id}>
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors flex items-center justify-between ${
                        selectedCategory === category.id ? 'bg-slate-600/30 text-slate-300' : 'text-slate-300 hover:bg-slate-700/50'
                      }`}
                    >
                        ...
                        {category.subcategories.map((sub) => (
                          <button
                            ...
                            className={`block w-full text-left px-3 py-1.5 rounded text-xs transition-colors ${
                              selectedSubcategory === sub ? 'text-slate-300' : 'text-slate-400 hover:text-slate-200'
                            }`}
                          >
                            {sub}
                          </button>
              ...
              <div className="space-y-3">
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-slate-500"
                    placeholder="От"
                  />
            ...
            <button
              ...
              className="w-full py-2 px-4 bg-slate-700/50 border border-slate-600 rounded-xl text-slate-300 hover:bg-slate-700 transition-colors text-sm"
            >
              Сбросить фильтры
            </button>
          </aside>
          ...
          <main className="flex-1">
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-4">
                <span className="text-slate-400 text-sm">Сортировать:</span>
                <select
                  value={sortBy}
                  onChange={(e) => setSortBy(e.target.value as any)}
                  className="px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-slate-500"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredServices.map((service) => (
                  <div
                    ...
                    className="group bg-slate-800/30 backdrop-blur-lg border border-slate-700 rounded-2xl overflow-hidden hover:border-slate-600 hover:shadow-lg transition-all duration-300 cursor-pointer"
                  >
